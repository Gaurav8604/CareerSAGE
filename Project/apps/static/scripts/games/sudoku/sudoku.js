...
function calculateSudokuPoints(solution, player) {
    let p = 0;
    for (let i = 0; i < 9; i++)
        for (let j = 0; j < 9; j++)
            if (player[i][j] == solution[i][j]) p++;
    for (let i = 0; i < 9; i++)
        if (player[i].every((v, j) => v == solution[i][j])) p += 10;
    for (let j = 0; j < 9; j++)
        if (player.map(r => r[j]).every((v, i) => v == solution[i][j])) p += 10;
    for (let r = 0; r < 9; r += 3)
        for (let c = 0; c < 9; c += 3)
            if (player.slice(r, r + 3).flat().sort().join() === solution.slice(r, r + 3).flat().sort().join()) p += 15;
    return p;
}
...
const intv = setInterval(function () {
    if (timeInSeconds > 0) {
        ...
        const score=calculateSudokuPoints(arrayToMatrix(ans), arrayToMatrix(game.board))
        fetch('/send_scores', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({'earned_score':score,'total_score':396})
        })
