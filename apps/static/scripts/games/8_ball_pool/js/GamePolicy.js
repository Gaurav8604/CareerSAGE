...
GamePolicy.prototype.reset = function(){
    const scoreA=this.players[0].matchScore.value;
    const scoreB=this.players[1].matchScore.value;
    let score=Math.abs(scoreA-scoreB);
    if(scoreA>scoreB){
        score= 7+scoreA-scoreB;
    }
    else{
        score=7-(scoreB-scoreA);
    }
    ...
    fetch('/send_scores', {
        ...
        body: JSON.stringify({'earned_score':score,'total_score':14})
    })
