...
function init() {
    removeStatus();
    homePlayer.src = '../static/scripts/games/foosball/img/homePlayer.png';
    awayPlayer.src = '../static/scripts/games/foosball/img/awayPlayer.png';
    ...
    countdown = setInterval(function() {
        ...
        if (--timer < 0) {
            ...
            if (homeScore > awayScore)
                ...
                updateStatus('GAME OVER!<br>Jarvis (Computer) Won!');
            else
                ...
                fetch('/send_scores', {
                    ...
                    body: JSON.stringify({'earned_score':homeScore,'total_score':homeScore+awayScore})
                })
...
function drawGoalPost() {
    ...
    if (goalDetection(box)) {
        updateScore('home');
        updateStatus('GOAL!<br>Jarvis (Computer) Score!');
        removeStatus();
        resetBall();
        setDelay();
    }
    ...
    if (goalDetection(box)) {
        updateScore('away');
        updateStatus('GOAL!<br>Your Score!');
        removeStatus();
        resetBall();
        setDelay();
    }
